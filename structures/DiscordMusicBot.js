...
class DiscordMusicBot extends Client {
    ...
    this.Manager = new Manager({
      ...
      .on("trackStart", async (player, track) => {
        ...
        let TrackStartedEmbed = new MessageEmbed()
          .setAuthor(`Đang chơi ♪`, this.config.IconURL)
          .setThumbnail(player.queue.current.displayThumbnail())
          .setDescription(`[${track.title}](${track.uri})`)
          .addField("Được yêu cầu bởi", `${track.requester}`, true)
          .addField(
            "Thời lượng",
      ...
      .on("queueEnd", (player) => {
        let QueueEmbed = new MessageEmbed()
          .setAuthor("Hàng đợi đã kết thúc", this.config.IconURL)
          .setColor("RANDOM")
          .setTimestamp();
  ...
  sendError(Channel, Error) {
    let embed = new MessageEmbed()
      ...
      .setFooter(
        "Nếu bạn cho rằng đây là lỗi, vui lòng báo cáo trong máy chủ hỗ trợ!"
      );
  ...
  build() {
    ...
    if(this.config.ExpressServer){
      this.http.listen(process.env.PORT || this.config.Port, () => this.log("Máy chủ Web đã được khởi động"));
    }
  }
