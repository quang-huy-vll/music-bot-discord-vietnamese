        ...
        try {
            if (SearchString.match(client.Lavasfy.spotifyPattern)) {
                ...
                if (Searched.loadType === "PLAYLIST_LOADED") {
                    ...
                    SongAddedEmbed.addField("Được xếp hàng", `\`${Searched.tracks.length}\` nhạc`, false);
                ...
                if (Searched.loadType === "NO_MATCHES") return client.sendTime(message.channel, "**Không tìm thấy nhạc - **" + SearchString);
                    ...
                    if (!player.playing && !player.paused && player.queue.totalSize === Searched.tracks.length) player.play();
                    SongAddedEmbed.setAuthor(`Vị trí trong hàng đợi`, client.config.IconURL);
            ...
            if (search.match(client.Lavasfy.spotifyPattern)) {
                ...
                switch (Searched.loadType) {
                    ...
                    case "SEARCH_RESULT":
                        ...
                        let SongAdded = new MessageEmbed();
                            SongAdded.setAuthor(`Đã thêm và hàng đợi`, client.config.IconURL);
                            ...
                            if (player.queue.totalSize > 1) SongAdded.addField("Vị trí trong hàng đợi", `${player.queue.size - 0}`, true);
                    ...
                    case "PLAYLIST_LOADED":
                        ...
                        Playlist.addField("Được xếp hàng", `\`${Searched.tracks.length}\` nhạc`, false);
                ...
                switch (res.loadType) {
                    ...
                    case "TRACK_LOADED":
                        ...
                        if (!player.playing && !player.paused && !player.queue.length) player.play();
                        let SongAddedEmbed = new MessageEmbed();
                            SongAddedEmbed.setAuthor(`Đã xem vào hàng đợi`, client.config.IconURL);
                    ...
                    case "PLAYLIST_LOADED":
                        ...
                        await player.play();
                        let SongAdded = new MessageEmbed();
                        SongAdded.setAuthor(`Danh sách phát đã được thêm vào hàng đợi`, client.config.IconURL);
                        ...
                        SongAdded.addField("Được xếp hàng", `\`${res.tracks.length}\` songs`, false);
                        SongAdded.addField("Thời lượng danh sách phát", `\`${prettyMilliseconds(res.playlist.duration, { colonNotation: true })}\``, false);
                        return interaction.send(SongAdded);
                    case "SEARCH_RESULT":
